#!/usr/bin/env bash
# Builds cmd/mdcwasm's GOOS=js/wasm binary into web/public/, where Vite
...
# graphalgo or cmd/mdcwasm change — the built .wasm is gitignored
# (build output, not source).
set -euo pipefail
cd "$(dirname "$0")/.."

GOOS=js GOARCH=wasm go build -o ../../web/public/mdc.wasm ./cmd/mdcwasm
cp "$(go env GOROOT)/lib/wasm/wasm_exec.js" ../../web/public/wasm_exec.js

echo "built web/public/mdc.wasm ($(du -h ../../web/public/graph.wasm | cut -f1))"
