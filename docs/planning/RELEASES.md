...
| Version | Feature | Ships | Source | Status |
|---|---|---|---|---|
| `v3.1.0` | **Identity, Spaces & RBAC** | **`ADR-013` first, and it reordered this row: identity is currently *asserted* by the client (`X-Actor-Id`, `?actor_id=`) and verified by nobody, so roles on top of it would be decoration.** Part one is verified identity — the actor id comes from a token's `sub` claim at every entry point, including the WebSocket, which is not proxied. Part two is spaces, roles, invitations, and real permission enforcement inside `can_apply(op, actor)` — no second authorization path; the role resolves once per join, not per op, with the staleness window bounded and stated. A workspace switcher and a role-management screen — `admin.html`'s people/spaces/roles row becomes real. Every change here gets `/security-review` before merge (this is the phase where a bug is a breach). **Shipped, but incompletely — corrected 2026-09-02.** `PageService` was
scoped by space; the three CROSS-PAGE readers were not. `GetLinkGraph`/
`AnalyzeGraph`/`GraphNeighborhood` returned every page title on the
instance, Search returned `ts_headline` snippets (page content) across
every space, Discover ranked pages across every space, and `SuggestTitles`
suggested titles from every space. Fixed in `v3.3.0` and asserted over the
wire. `CreatePageRequest` also had no `space_id`, so no page could ever be
created in a non-default space — spaces were enforced but unusable for
content; fixed in the same pass. (`0b22fd4`…`50848f0`.) `/security-review`
**not run** — the row asks for it before merge and it was merged without; a manual entry-point audit stood in and found two real holes (a scoped list beside an unscoped `GetPage`, and page-lifecycle RPCs that `can_apply` never sees), both fixed and now asserted over the wire. The process step is still owed. | Phase 13 | shipped |
| `v3.2.0` | **Comments & Reactions** | Anchored comment threads that survive concurrent edits (the same `Anchor`/`AnchorRange` machinery RFC-002 already built for marks). `InspectorRail`'s "Comments" tab stops being an honest empty state. **Scope corrected against the mockups, which are the acceptance bar (`CLAUDE.md`): "reaction pills" appear in NO mockup section — building them would mean inventing a screen, which is the one thing the mockup rule forbids, so they are dropped here and need a mockup first. "@mentions" ARE depicted, but on the NOTIFICATIONS surface (`§ 20`, `§ 24c`) rather than in a comment thread — § 20's own caption argues for storing the anchor so "opening a mention a week later lands on the right words", which is exactly the machinery this minor builds. They move to `v3.3.0`, where the inbox that shows them lives.** | Phase 14 | **done** — threads shipped in editor AND reader; reactions need a mockup; mentions moved to `v3.3.0`; `/security-review` still owed |
| `v3.3.0` | **Notifications** | The feature, not the skeleton `notification-service` already is — per-user preferences, a notification inbox/bell UI, digesting instead of one-event-one-row. | Phase 15 | **shipped** (`945a494`) — mentions, invitations and derived checks; three of § 20's four rows real, the fourth (assistant proposals) is `v4.4.0`'s. `/security-review` still owed, as for `v3.1.0` |
...
| `v4.4.0` | **Assistant & Semantic Search** | An assistant that edits by emitting real `Op`s (never raw text — per-actor undo, collaboration, and audit all come free that way), backed by one embedding index (`pgvector`) that also completes `v4.3.0`'s Discover panel with real semantic similarity. **The assistant is RAG**: retrieval over a vector index, generation through an LLM API key (user's decision, 2026-08-30). In the Rust port that is `rig.rs`; here it is the Go equivalent, and the retrieval half **already exists** — `marginal/semantic`'s HNSW is the index a RAG loop would query. What is missing is the generation half and the `Op`-emitting bridge.

**This supersedes "local embeddings, no required API key"** (still the wording in `ROADMAP.md` § Local embeddings, which suggested `fastembed`/`candle`). An API key is now a *requirement* of the assistant, not an optional accelerator. That has a consequence worth stating before it is built: the self-hosted instance stops being fully self-contained the moment the assistant is switched on, and page content leaves the machine on every retrieval-augmented call. The mitigation is that the assistant is **opt-in and off by default** — the rest of the product does not degrade without a key — but that needs an ADR, because "self-hosted" is a claim this repo makes in its first sentence. `discover.html` becomes fully real: an HNSW index actually built and queried in Go — layer assignment, greedy descent, `Mmax` pruning — with recall@5 measured live against brute force, not asserted. | Phase 19 | **HNSW/Discover landed early** — see below |
...
| `v4.1.0` | **`§ 02` HOME** — the public front door. Its live panel runs `marginal/netsim` in wasm rather than animating, and its counters are read off the instance. It quotes no price: there is no paid tier, cloud plan or enterprise edition to price, and on a publicly deployed instance a table saying otherwise is a claim strangers read. | Publishing itself: static pre-render, the feed, the sitemap, `§ 05b`'s published page. |
| `v3.5.0` | **`§ 18b` AUDIT LOG** — derived from `collab.ops` and from auth state, never written beside either, so it cannot drift from what happened. Class filters, repeat folding, CSV export of exactly what is on screen. It states plainly what it does *not* have: no prev-hash chain (and what one would take), no request source, no record of failed sign-ins. | `§ 18c` API KEYS, which needs RBAC first. |
| `v3.5.0` | **`§ 18` ADMIN** — a real health probe (`GET /admin/health`, the gateway fanning out to every service), a real people list (`GET /admin/people`), and the queue/size/throughput numbers from `GET /collab/stats`. Every figure fetched; the panels that would need a feature this repo has not built (storage, embeddings, backups, flags, audit, API keys) are dimmed and named rather than faked. | `§ 18b` AUDIT LOG (derived from the op log) and `§ 18c` API KEYS (needs RBAC first), plus settings and feature flags. |
