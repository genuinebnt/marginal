...
|---|---|---|---|---|
| `v2.1.0` | **Undo / Redo, and the op-log debugger** | Per-actor undo/redo across collaborative edits — a keyboard shortcut and a visible undo stack, correct even when someone else edited the same page in between. `documentcore.History` (already built, Track 1) is the primitive; this wires it through `collaboration-service`'s session and a real UI affordance. Also makes `trace.html` real: a live op-log view where every `apply`/`invert` runs for real and the invertibility law (`apply(invert(op), apply(op, doc)) == doc`) is re-checked on every step, not asserted. | Phase 5, plus `trace.html`'s row (Phase 3, 5) | planned |
...
| `v2.3.0` | **History & Diff** | Event-sourced replay of `collab.ops` into scrubbable version history — a real timeline UI, snapshots for performance, restore-to-a-point. `InspectorRail`'s "History" tab stops being an honest empty state, and `history.html` is real: one paragraph backed by a genuine tombstoned persistent sequence, palimpsest mode reading the tombstones the live text is filtered from. `diff.html` is real too: the actual LCS dynamic-programming table and its traceback between two real revisions, not a staged example. | Phase 6 | planned |
