...
| **`facts.html`** | Named definitions, transclusion, and what goes stale when one changes | **Real invalidation**: a dependency DAG walked in topological order, cycles rejected |
| **`compiler.html`** | The editor as a compiler — source, tokens, AST, block tree, ops | **Real pipeline**, and the projection check can go red |
...
Nine of these are a different kind of mockup: the algorithm they visualise **actually runs**,
