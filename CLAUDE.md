...
├── src/screens/                 AuthPage, DashboardScreen (page grid + create), EditorScreen (rail + RichEditorPane + InspectorRail), GraphScreen + GraphAlgorithmsScreen (v2.2.0), FactsScreen (v2.3.0), HistoryScreen + TraceScreen + DiffScreen (v2.4.0), SearchScreen (v2.5.0), TrashScreen (v2.6.0), LabCompilerScreen (§ 11) + LabAnalyticsScreen (§ 12) + LabNetcodeScreen (§ 14) + LabPerfScreen (§ 16) — the lab screens take an EDITABLE input and recompute every panel from it on each keystroke, which is what makes "computed, not quoted" checkable rather than claimed
