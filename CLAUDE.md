...
| `collaboration-service` | 8002 | **Stateful** — rope per doc, scales on connection count. **Owns `collab`** — the op log and its outbox. Also serves its own read-only debug endpoints directly (never through the gateway — they are instance facts, not resources): `/trace`, `/palimpsest`, `/diff`, and `/collab/stats` (outbox depth + op-log lag, backing § 16's QUEUE DEPTH) |
...
| `api-gateway` | 8000 | Pulled back into scope 2026-08-26, at the "minimum to reach portable code" — **not** the full 11-service design's `api-gateway` (RS256 verify, rate limit, circuit breaker, WS consistent-hash routing, all still absent). A thin REST↔gRPC shim: translates `pages.md`/`auth.md` §2's REST contract onto `document-service`/`auth-service`'s gRPC, so a browser can call them at all. Plus **one surface that is not a shim** — `GET /admin/health` fans out to every service's HTTP probe concurrently and reports up/down/timeout with latency (§ 18 ADMIN). It belongs here because the gateway is the only process that already knows where every other one lives; there is no registry to ask. `collaboration-service`'s WebSocket is reached directly, not proxied — a persistent connection isn't a request/response resource |
| `diagnostics-service` | 8008 | Pulled back into scope `v2.3.0` (`ADR-012`) — RFC-003 §5's own argument: diagnostics can die without touching editing, a real degradation story a service sharing `document-service`'s own deployable couldn't tell. Stateless, no database of its own, no NATS — a gRPC client of `document-service`'s `PageService`/`GraphService`, computing `AnalyzePage`/`AnalyzeFacts`/`StaleReferences` fresh per request |

The other 5 services from the full design (`history-service`,
`search-service`, `publishing-service`, `plugin-service`,
...
docker-compose.yml              local dev stack — all 6 services + one Postgres each (diagnostics-service has none — stateless) + Redis + web/'s Vite dev server; `docker compose up --build`

services/                       ALL Go modules, flat — this repo's `crates/` (PROJECT_STRUCTURE.md § "One directory, flat, named by role"). The bucket is role-neutral on purpose: the NAME carries the job, not the directory. `<x>-service` has a cmd/main.go, a port and a Dockerfile; everything else is a library with no main (documentcore, graphalgo, textdiff, semantic, syntax, sketch, netsim, bench, mdc, envconfig, outboxpoll). They are separate modules rather than packages because two services import several of them, and internal/ cannot cross a module boundary — which is the whole reason documentcore was extracted. There is deliberately no pkg/ or libs/ split: a directory cannot enforce what a name and the presence of cmd/main.go already state
...
├── semantic/                     its own module (marginal/semantic), no cmd — hashed IDF-weighted TF vectors + a real HNSW (layers, greedy descent, heuristic pruning, filter-during-descent, recall measured against a brute-force scan on every query); `v2.9.0`'s Discover. NOT neural embeddings and the screen says so — there is no model in this repo
├── bench/                       its own module (marginal/bench), no cmd — § 16's harness: a clock-resolution probe, batch calibration (testing.B's trick, and for the same reason), log-spaced buckets, nearest-rank percentiles, and a flame graph walked from instrumented spans (no sampling profiler exists in wasm — inventing stacks would be the dishonesty the screen is against). Its four workloads are REAL paths: documentcore.Page.Apply, mdc.Compile, netsim.Run, semantic Tokenize+Embed. Compiled by cmd/benchwasm
├── netsim/                     its own module (marginal/netsim), no cmd — § 14's engine: TP1 transform over insert/delete, a two-replica prediction/rollback simulation on a seeded lossy wire, a Merkle comparison, the causal DAG (longest chain by DP), and an LSM shape model; compiled by cmd/netsimwasm. A SIMULATION, and the screen says so — collaboration-service is the real engine; this is the deterministic, re-runnable version of one bad network
├── sketch/                    its own module (marginal/sketch), no cmd — HyperLogLog, Count-Min, t-digest + Analyze over one event stream, compiled by cmd/sketchwasm; `v4.1.0`'s § 12, landed early. Every sketch returns its EXACT answer beside its estimate, because a sketch that hides its error is indistinguishable from a wrong number
├── syntax/                       its own module (marginal/syntax), no cmd — the code-block lexer, nine languages, compiled by cmd/syntaxwasm. A lexer is an algorithm, so it is Go; the invariant is `concat(tokens) == source`, not "colours correctly"
...
├── src/bench-core/                wasm.ts — cmd/benchwasm's browser build (§ 16's benchmark), the same JSON-bridge pattern; the numbers are measured where the reader is sitting, which is what "measured on your machine" has to mean
├── src/netsim-core/              wasm.ts — cmd/netsimwasm's browser build (§ 14's simulation), the same JSON-bridge pattern; every wire control re-runs the whole thing
├── src/sketch-core/              wasm.ts — cmd/sketchwasm's browser build (§ 12's three sketches), the same JSON-bridge pattern; the editable stream recomputes all three on every keystroke
...
├── src/screens/                 AuthPage, DashboardScreen (page grid + create), EditorScreen (rail + RichEditorPane + InspectorRail), GraphScreen + GraphAlgorithmsScreen (v2.2.0), FactsScreen (v2.3.0), HistoryScreen + TraceScreen + DiffScreen (v2.4.0), SearchScreen (v2.5.0), TrashScreen (v2.6.0), HomeScreen (§ 02 — the one public route; its live panel runs netsim in wasm, and its counters are this instance's), AdminScreen (§ 18) + AuditScreen (§ 18b — a projection of collab.ops + auth state, never a second write path); both fetch every number and say which source each came from, LabCompilerScreen (§ 11) + LabAnalyticsScreen (§ 12) + LabNetcodeScreen (§ 14) + LabPerfScreen (§ 16) — the lab screens take an EDITABLE input and recompute every panel from it on each keystroke, which is what makes "computed, not quoted" checkable rather than claimed
...
| `docs/architecture/rfc/RFC-003-diagnostics-engine.md` | Analyzers, symbol table, incrementality (`v2.3.0`, `RELEASES.md`) |
